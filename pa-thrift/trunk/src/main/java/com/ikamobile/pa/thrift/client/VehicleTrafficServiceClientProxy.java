...
import com.ikamobile.pa.thrift.server.acceptor.VehicleTrafficThriftService;
 ...
 */
@Slf4j
public class VehicleTrafficServiceClientProxy implements MethodInterceptor {

    private static final String SERVER_NAME = "VehicleTrafficThriftService";

    private OpenThriftClient openThriftClient;

    //要代理的原始对象
    private ThreadLocal<VehicleTrafficThriftService.Client> client = new ThreadLocal<>();
...
//        }
    ...
    public VehicleTrafficThriftService.Client createProxy() {
        ...
        try {
            ...
            enhancer.setSuperclass(VehicleTrafficThriftService.Client.class);// 设置代理目标
            enhancer.setCallback(this);// 设置回调
            Class[] classes = {TProtocol.class};
            TProtocol[] object = {tProtocol};
            return (VehicleTrafficThriftService.Client) enhancer.create(classes, object);
        } catch (Exception e) {
