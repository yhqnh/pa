 ...
 */
@Slf4j
public class TaskServiceClientProxy implements MethodInterceptor {

    private static final String SERVER_NAME = "TaskService";

    private OpenThriftClient openThriftClient;

    //要代理的原始对象
    private ThreadLocal<TaskThriftService.Client> client = new ThreadLocal<>();
...
//        }
    ...
    public TaskThriftService.Client createProxy() {
        ...
        try {
            ...
            enhancer.setSuperclass(TaskThriftService.Client.class);// 设置代理目标
            enhancer.setCallback(this);// 设置回调
            Class[] classes = {TProtocol.class};
            TProtocol[] object = {tProtocol};
            return (TaskThriftService.Client) enhancer.create(classes, object);
        } catch (Exception e) {
