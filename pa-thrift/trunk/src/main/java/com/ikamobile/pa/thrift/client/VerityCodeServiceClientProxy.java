...
import com.ikamobile.pa.thrift.server.acceptor.VerityCodeThriftService;
 ...
 */
@Slf4j
public class VerityCodeServiceClientProxy implements MethodInterceptor {

    private static final String SERVER_NAME = "VerityCodeThriftService";

    private OpenThriftClient openThriftClient;

    //要代理的原始对象
    private ThreadLocal<VerityCodeThriftService.Client> client = new ThreadLocal<>();
...
//        }
    ...
    public VerityCodeThriftService.Client createProxy() {
        ...
        try {
            ...
            enhancer.setSuperclass(VerityCodeThriftService.Client.class);// 设置代理目标
            enhancer.setCallback(this);// 设置回调
            Class[] classes = {TProtocol.class};
            TProtocol[] object = {tProtocol};
            return (VerityCodeThriftService.Client) enhancer.create(classes, object);
        } catch (Exception e) {
