...
import com.ikamobile.pa.thrift.server.acceptor.UserThriftService;
 ...
 */
@Slf4j
public class UserServiceClientProxy implements MethodInterceptor {

    private static final String SERVER_NAME = "UserThriftService";

    private OpenThriftClient openThriftClient;

    //要代理的原始对象
    private ThreadLocal<UserThriftService.Client> client = new ThreadLocal<>();
...
//        }
    ...
    public UserThriftService.Client createProxy() {
        ...
        try {
            ...
            enhancer.setSuperclass(UserThriftService.Client.class);// 设置代理目标
            enhancer.setCallback(this);// 设置回调
            Class[] classes = {TProtocol.class};
            TProtocol[] object = {tProtocol};
            return (UserThriftService.Client) enhancer.create(classes, object);
        } catch (Exception e) {
