...
import com.ikamobile.pa.thrift.server.acceptor.PositionThriftService;
 ...
 */
@Slf4j
public class PositionServiceClientProxy implements MethodInterceptor {

    private static final String SERVER_NAME = "PositionService";

    private OpenThriftClient openThriftClient;

    //要代理的原始对象
    private ThreadLocal<PositionThriftService.Client> client = new ThreadLocal<>();
...
//        }
    ...
    public PositionThriftService.Client createProxy() {
        ...
        try {
            ...
            enhancer.setSuperclass(PositionThriftService.Client.class);// 设置代理目标
            enhancer.setCallback(this);// 设置回调
            Class[] classes = {TProtocol.class};
            TProtocol[] object = {tProtocol};
            return (PositionThriftService.Client) enhancer.create(classes, object);
        } catch (Exception e) {
