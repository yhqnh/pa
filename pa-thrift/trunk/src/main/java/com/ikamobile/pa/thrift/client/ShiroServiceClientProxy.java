...
import com.ikamobile.pa.thrift.server.acceptor.ShiroThriftService;
 ...
 */
@Slf4j
public class ShiroServiceClientProxy implements MethodInterceptor {

    private static final String SERVER_NAME = "ShiroThriftService";

    private OpenThriftClient openThriftClient;

    //要代理的原始对象
    private ThreadLocal<ShiroThriftService.Client> client = new ThreadLocal<>();
...
//        }
    ...
    public ShiroThriftService.Client createProxy() {
        ...
        try {
            ...
            enhancer.setSuperclass(ShiroThriftService.Client.class);// 设置代理目标
            enhancer.setCallback(this);// 设置回调
            Class[] classes = {TProtocol.class};
            TProtocol[] object = {tProtocol};
            return (ShiroThriftService.Client) enhancer.create(classes, object);
        } catch (Exception e) {
