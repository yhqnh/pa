...
import com.ikamobile.pa.thrift.server.acceptor.DriverThriftService;
 ...
 */
@Slf4j
public class DriverServiceClientProxy implements MethodInterceptor {

    private static final String SERVER_NAME = "DriverThriftService";

    private OpenThriftClient openThriftClient;

    //要代理的原始对象
    private ThreadLocal<DriverThriftService.Client> client = new ThreadLocal<>();
...
//        }
    ...
    public DriverThriftService.Client createProxy() {
        ...
        try {
            ...
            enhancer.setSuperclass(DriverThriftService.Client.class);// 设置代理目标
            enhancer.setCallback(this);// 设置回调
            Class[] classes = {TProtocol.class};
            TProtocol[] object = {tProtocol};
            return (DriverThriftService.Client) enhancer.create(classes, object);
        } catch (Exception e) {
