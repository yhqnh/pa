...
import com.ikamobile.pa.thrift.server.acceptor.VehicleThriftService;
 ...
 */
@Slf4j
public class VehicleServiceClientProxy implements MethodInterceptor {

    private static final String SERVER_NAME = "VehicleThriftService";

    private OpenThriftClient openThriftClient;

    //要代理的原始对象
    private ThreadLocal<VehicleThriftService.Client> client = new ThreadLocal<>();
...
//        }
    ...
    public VehicleThriftService.Client createProxy() {
        ...
        try {
            ...
            enhancer.setSuperclass(VehicleThriftService.Client.class);// 设置代理目标
            enhancer.setCallback(this);// 设置回调
            Class[] classes = {TProtocol.class};
            TProtocol[] object = {tProtocol};
            return (VehicleThriftService.Client) enhancer.create(classes, object);
        } catch (Exception e) {
